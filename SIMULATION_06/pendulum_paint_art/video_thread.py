#!/usr/bin/env python3
"""
Video Generation Thread for Pendulum Paint Art
================================================
...
class VideoGenerationThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if total_frames == 0:
                self.error.emit("No frames found to generate video")
                return
            ...
            cmd = [
                'ffmpeg',
                '-y',  # Overwrite output
                '-framerate', str(self.sim_fps),  # Input framerate
                '-i', str(self.frames_folder / 'frame_%06d.png'),
                '-vf', f'fps={self.output_fps}',  # Output framerate
